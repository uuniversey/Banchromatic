<template>
  ...
  <table class="product_list">
    ...
    <tbody v-if="search === '전체보기'">
      <tr v-for="product in depositProductList" class="line">
    ...
    <tbody v-else>
      <tr v-for="list in searchList" class="line">
        ...
        <td>
          <button @click="showDetail(list.fin_prdt_cd)" class="info_btn">상세정보</button>
        </td>
      </tr>
    </tbody>
  </table>
...
<script setup>
  ...
  onMounted(() => {
    axios({
      ...
      .then((res) =>{
        ...
        depositProductList.value.forEach(element => {
  ...
  const showSearchList = function () { 
    const list = depositProductList.value.filter(element => {
  ...
  const getOption = function(standard, num) {
    if (depositOptionList.value) {
      const info = depositOptionList.value.find((option) => {
